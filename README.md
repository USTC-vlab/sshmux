...
A session ending is not among the errors: a client saying it is done is how sessions end, and `event.reason` says so on the record instead. A session that ended without either of its connections saying so is reported beside that record, at `WARN` rather than `ERROR` — the session happened, and it is not `sshmux` that failed — naming `error.type` and `error.message` alongside the `session.id` that joins the two.
